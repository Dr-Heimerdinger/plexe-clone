...
def validate_training_prerequisites(
    dataset_module_path: str = None,
    task_module_path: str = None,
) -> Dict[str, Any]:
    """
    Validates that all prerequisites for training are in place.
    
    CRITICAL: This tool MUST be called before generating training code.
    Both dataset.py and task.py files are REQUIRED for GNN training.
    
    Checks:
    1. Dataset module exists (dataset.py) - generated by DatasetBuilder
    2. Task module exists (task.py) - generated by TaskBuilder
    3. Required dependencies are installed
    
    Args:
        dataset_module_path: Path to the dataset module. If None, will auto-detect
                            from ObjectRegistry or working directory.
        task_module_path: Path to the task module. If None, will auto-detect
                         from ObjectRegistry or working directory.
        
    Returns:
        Dictionary with validation results including:
        - status: "valid" or "invalid"
        - issues: List of critical issues that must be resolved
        - warnings: List of non-critical warnings
        - dataset_path: Resolved path to dataset module
        - task_path: Resolved path to task module
    """
    issues = []
    warnings = []
    object_registry = ObjectRegistry()
    
    # Auto-detect paths from registry if not provided
    if dataset_module_path is None:
        try:
            dataset_info = object_registry.get(dict, "database_code")
            dataset_module_path = dataset_info.get("export_path")
        except KeyError:
            pass
    
    if task_module_path is None:
        try:
            task_info = object_registry.get(dict, "task_code")
            task_module_path = task_info.get("export_path")
        except KeyError:
            pass
    
    # Try to infer from working directory if still not found
    working_dir = None
    try:
        working_dir = object_registry.get(str, "working_dir")
    except KeyError:
        try:
            working_dir = object_registry.get(str, "current_chat_session_dir")
        except KeyError:
            pass
    
    if working_dir:
        if dataset_module_path is None:
            potential_dataset = os.path.join(working_dir, "dataset.py")
            if os.path.exists(potential_dataset):
                dataset_module_path = potential_dataset
        
        if task_module_path is None:
            potential_task = os.path.join(working_dir, "task.py")
            if os.path.exists(potential_task):
                task_module_path = potential_task
    
    # =========================================================================
    # CRITICAL CHECK: Both dataset.py and task.py MUST exist
    # =========================================================================
    
    # Check dataset module
    if dataset_module_path is None:
        issues.append(
            "CRITICAL: dataset.py not found! "
            "DatasetBuilder agent must create dataset.py before GNN training can begin. "
            "Please run DatasetBuilder first to generate the Dataset class."
        )
    elif not os.path.exists(dataset_module_path):
        issues.append(
            f"CRITICAL: Dataset module not found at '{dataset_module_path}'! "
            "The DatasetBuilder agent must create this file before training."
        )
    else:
        # Verify it's a valid Python file with a Dataset class
        try:
            with open(dataset_module_path, 'r') as f:
                content = f.read()
                if 'class ' not in content or 'Dataset' not in content:
                    warnings.append(
                        f"Dataset file exists but may not contain a valid Dataset class: {dataset_module_path}"
                    )
        except Exception as e:
            warnings.append(f"Could not read dataset file for validation: {e}")
    
    # Check task module
    if task_module_path is None:
        issues.append(
            "CRITICAL: task.py not found! "
            "TaskBuilder agent must create task.py before GNN training can begin. "
            "Please run TaskBuilder first to generate the Task class."
        )
    elif not os.path.exists(task_module_path):
        issues.append(
            f"CRITICAL: Task module not found at '{task_module_path}'! "
            "The TaskBuilder agent must create this file before training."
        )
    else:
        # Verify it's a valid Python file with a Task class
        try:
            with open(task_module_path, 'r') as f:
                content = f.read()
                if 'class ' not in content or 'Task' not in content:
                    warnings.append(
                        f"Task file exists but may not contain a valid Task class: {task_module_path}"
                    )
        except Exception as e:
            warnings.append(f"Could not read task file for validation: {e}")
    ...
    files_summary = {
        "dataset.py": {
            "path": dataset_module_path,
            "exists": dataset_module_path is not None and os.path.exists(dataset_module_path),
            "required": True,
            "created_by": "DatasetBuilder agent"
        },
        "task.py": {
            "path": task_module_path,
            "exists": task_module_path is not None and os.path.exists(task_module_path),
            "required": True,
            "created_by": "TaskBuilder agent"
        }
    }
    ...
    return {
        ...
        "working_dir": working_dir,
        "required_files": files_summary,
        "message": (
            "All prerequisites validated successfully. Ready for GNN training."
            if len(issues) == 0 else
            f"CANNOT PROCEED: {len(issues)} critical issue(s) must be resolved before training."
        )
    }
