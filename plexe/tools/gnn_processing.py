...
def load_training_data(
    ...
    2. If not in registry, attempts to load from disk (data_dir or .plexecache)
    3. Mapping of Entity IDs (from DB) to Graph Indices (using EntityMapper)
    4. Conversion to PyTorch tensors
    5. (Optional) Loading of point-in-time correct features for seed nodes
    ...
    Args:
        ...
        entity_id_column: Optional name of the entity ID column in the training table.
                          If not provided, tries to auto-detect.
        data_dir: Optional directory path to load CSV/Parquet files from if not found in registry.
        ...
        def process_dataset(dataset_key: str) -> Optional[Dict[str, torch.Tensor]]:
            try:
                ...
                try:
                    ...
                    # Try loading from disk if data_dir is provided or fallback to default
                    search_dirs = []
                    if data_dir:
                        search_dirs.append(data_dir)
                    
                    # Also check .plexecache/ or current directory
                    search_dirs.append(".plexecache")
                    search_dirs.append(".")
                    ...
                    for d in search_dirs:
                        ...
                        for ext in [".csv", ".parquet"]:
                            fpath = os.path.join(d, f"{dataset_key}{ext}")
                            if os.path.exists(fpath):
                                logger.info(f"Found {dataset_key} at {fpath}. Loading and registering.")
                                try:
                                    if ext == ".csv":
                                    ...
                                    dataset_obj = DatasetAdapter.coerce(df)
                                    object_registry.register(TabularConvertible, dataset_key, dataset_obj, overwrite=True)
                                    found = True
                                    break
                                except Exception as e:
                    ...
                    if not found:
                        logger.warning(f"Dataset {dataset_key} not found in registry or disk.")
                        return None
                ...
                # 1. Try provided entity_id_column
                ...
                # 3. Try case-insensitive match for provided entity_id_column
                if not id_col and entity_id_column:
                     for col in df.columns:
                        if col.lower() == entity_id_column.lower():
                            id_col = col
                            break


                
                # 4. Fallback: check if any column values overlap with mapper keys
                if not id_col:
                    # Get a sample of keys from mapper to check types
                    sample_keys = list(id_to_idx.keys())[:5]
                    sample_key_type = type(sample_keys[0]) if sample_keys else None
                    
                    for col in df.columns:
                        if col == label_column or col == timestamp_column:
                            continue
                        ...
                        # Type conversion match (str <-> int)
                        try:
                            if isinstance(sample_val, (int, float)) and str(int(sample_val)) in id_to_idx:
                                id_col = col
                                break
                            if isinstance(sample_val, str) and sample_val.isdigit() and int(sample_val) in id_to_idx:
                                id_col = col
                                break
                        except:
                            pass
                ...
                # Ensure type consistency for mapping
                mapping_keys = set(id_to_idx.keys())
                
                # Check if we need to convert column type to match keys
                sample_val = df[id_col].iloc[0]
                sample_key = next(iter(mapping_keys))
                
                if type(sample_val) != type(sample_key):
                    try:
                        if isinstance(sample_key, str):
                            df[id_col] = df[id_col].astype(str)
                            # Handle float-string conversion (e.g. "1.0" -> "1")
                            if isinstance(sample_val, float):
                                df[id_col] = df[id_col].apply(lambda x: str(int(float(x))))
                        elif isinstance(sample_key, int):
                            df[id_col] = df[id_col].astype(int)
                    except Exception as e:
                        logger.warning(f"Type mismatch between ID column {id_col} ({type(sample_val)}) and mapper keys ({type(sample_key)}). Conversion failed: {e}")

                valid_mask = df[id_col].isin(mapping_keys)
                if (~valid_mask).any():
